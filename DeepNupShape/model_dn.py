...
1. StokenAttention mechanism for shape feature extraction
...
5. Concatenation AFTER both paths are fully processed
"""
...
class StokenAttention(tf.keras.layers.Layer):
    ...
    Reduces spatial attention computation by grouping tokens into stokens.
    
    This allows efficient attention computation on shape feature maps.
    """
    
    def __init__(self, num_heads=8, stoken_size=(4, 4), **kwargs):
    ...
    def build(self, input_shape):
        C = input_shape[-1]
        self.query = tf.keras.layers.Conv2D(C, 1)
        self.key = tf.keras.layers.Conv2D(C, 1)
        self.value = tf.keras.layers.Conv2D(C, 1)
        self.proj = tf.keras.layers.Conv2D(C, 1)
    ...
    def call(self, x):
        ...
        q_flat = tf.reshape(q, [B, -1, C])
        k_flat = tf.reshape(k, [B, -1, C])
        v_flat = tf.reshape(v, [B, -1, C])
        
        # Scaled dot-product attention
        scale = tf.math.rsqrt(tf.cast(C, tf.float32))
        scores = tf.matmul(q_flat, k_flat, transpose_b=True) * scale  # (B, H*W, H*W)
        attn_weights = tf.nn.softmax(scores, axis=-1)
        
        # Apply attention to values
        attn_output = tf.matmul(attn_weights, v_flat)  # (B, H*W, C)
...
# ============================================================================
# SHAPE PROCESSING PATH with StokenAttention
# ============================================================================

def shape_path_with_attention(input_layer, filters=64, dropout_rate=0.5, l2_beta=1e-3):
    """
    Shape feature processing with StokenAttention (MLSNet-inspired)
    ...
    # Conv2D layers
    x = Conv2D(filters, (3, 3), padding='same', kernel_regularizer=l2(l2_beta))(x)
    x = Activation('relu')(x)
    x = BatchNormalization()(x)
    
    # Apply StokenAttention
    x = StokenAttention(num_heads=4, stoken_size=(2, 4))(x)
    
    # Additional Conv2D
    x = Conv2D(filters, (5, 5), padding='same', kernel_regularizer=l2(l2_beta))(x)
    x = Activation('relu')(x)
    x = BatchNormalization()(x)
    x = MaxPooling2D(pool_size=(1, 2))(x)
    x = Dropout(dropout_rate)(x)
    
    # Reshape back to 1D for LSTM: (batch, 5, ?, filters) → suitable for LSTM
    # Flatten spatial dimensions
    batch_size = tf.shape(x)[0]
    x = GlobalAveragePooling2D()(x)  # Reduce to (batch, filters)
    
    # Expand for LSTM processing
    x = Reshape((1, filters))(x)  # (batch, 1, filters)
...
def dn_with_shapes_mlsnet(
    ...
    shape_filters=64,               # Shape path filters
    lstm_units=50,                  # LSTM units
    lstm_units_shape=32,            # LSTM units for shapes
    ...
    """
    Enhanced DeepNup model combining three input pathways:
    
    1. One-hot sequence (147, 4) → Multi-branch Conv1D → LSTM
    2. PseTNC features (64, 1) → Multi-branch Conv1D → LSTM
    3. Shape features (5, 147) → Conv2D + StokenAttention → Bi-LSTM
    ...
    Args:
        ...
        lstm_units_shape: Number of LSTM units for shape path
    ...
    # ====================================================================
    # PATH 3: Shape features with Conv2D + StokenAttention + Bi-LSTM
    # ====================================================================
    x3 = shape_path_with_attention(input3, filters=shape_filters,
                                   dropout_rate=dropout_rate, l2_beta=l2_beta)
    
    # Bi-LSTM for shape processing
    x3 = LSTM(lstm_units_shape, return_sequences=True, go_backwards=False,
              kernel_regularizer=l2(l2_beta))(x3)
    x3 = MaxPooling1D(pool_size=2)(x3)
    x3 = Dropout(dropout_rate)(x3)
    
    x3_backward = LSTM(lstm_units_shape, return_sequences=True, go_backwards=True,
                       kernel_regularizer=l2(l2_beta))(x3)
    x3_backward = MaxPooling1D(pool_size=2)(x3_backward)
    x3_backward = Dropout(dropout_rate)(x3_backward)
    
    # Combine forward and backward
    x3 = Concatenate(axis=-1)([x3, x3_backward])
    x3 = LSTM(lstm_units_shape, return_sequences=False,
              kernel_regularizer=l2(l2_beta))(x3)
    x3 = Reshape((1, lstm_units_shape))(x3)
